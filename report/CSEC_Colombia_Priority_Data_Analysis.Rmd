---
title: "CSEC in Colombia: Priority Stratum- Data Analysis"
...
Below is the code and output for the analysis of the priority 2018 Colombia VACS sample. I've annotated the steps I've taken for convenience.
...
The cleaned data-set ready for analysis is saved under '/data' as "Colombia_VACS_Priority_data_CLEANED.rds". To review the process used for reviewing and preparing the raw data, please review the RMarkdown report "/report/CSEC_Colombia_Priority_Data_Clean-Prep.Rmd". 

# **1. Load Data**

```{r loadData}
df <- readRDS(here::here("data", "Colombia_VACS_Priority_data_CLEANED.rds"))
```
...
calculatePropGrouped <- function(var, group, design, filter_expr = NULL) {
  ...
  tryCatch(
    {
      # Apply filter and drop NAs
      filtered_design <- design %>%
        { if (rlang::quo_is_null(f)) . else filter(., !!f) } %>% 
        drop_na(!!sym(var), !!sym(group))
      
      # Calculate proportions by group
      result <- filtered_design %>%
        group_by(!!sym(group)) %>%  # Only group by the grouping variable
        summarise(
          # Calculate proportion of each level within each group
          Prop = survey_prop(na.rm = TRUE, prop_method = "xlogit", vartype = c("se", "ci", "cv")),
          .groups = 'drop'
        ) %>%
        ...
        mutate(
          ...
          `Variable Label` = var_label,
          `Group Label` = group_label,
          `Variable Levels` = var  # This might need adjustment based on your data structure
        ) %>%
        # Rename columns
        rename(
          `Group Levels` = !!sym(group),
        ...
        # Recode values and convert to percentages
        mutate(
          `Variable Levels` = recode(as.character(`Variable Levels`), `0` = "No", `1` = "Yes"),
          `Group Levels` = recode(as.character(`Group Levels`), `0` = "No", `1` = "Yes"),
          Prop = Prop * 100,
        ...
        # Reorder columns
        select(Variable, `Variable Label`, `Grouping Variable`, `Group Label`, 
               `Group Levels`, `Variable Levels`, everything())
    ...
    error = function(e) {
      ...
      tibble(
        ...
        Prop = NA_real_,
        `Prop (SE)` = NA_real_,  # Fixed: was `Prop(SE)` - missing space
        `Prop (Lower CI 95%)` = NA_real_,
        `Prop (Upper CI 95%)` = NA_real_,
        `Prop (CV)` = NA_real_   # Fixed: was `Prop(CV)` - missing space
      )
    }
  )
}
...
## a) Subset of sexually active respondents Ages 13 to 17 & transactional sex variables

```{r PropCountCSECAges13to17, asis = TRUE}
...
CSECProp <- c("CSEC") %>% 
  ...
  list_rbind    
createKable(CSECProp, "Proportions of CSEC Among Sexually Active Respondenets Ages 13-17 who Answered Question about Transactional Sex")
...
createKable(CSECCount, "Counts of CSEC Responses among Sexually Active Respondenets Ages 13-17 who Answered Question about Transactional Sex")

```

```{r MeansCSECVarsAge13to17, asis = TRUE}
...
createKable(CSECAgeDescriptives, "Mean Ages of Victim and Perpetrator for Sexually Active Respondents Ages 13-17 who Reported Transactional Sex (CSEC)")
...
```{R CountsCSECVarsAges13to17, asis = TRUE}

CSECCount <- c("SexTransactType", "SexTransactTourist") %>% 
    map( ~ calculateCount(
    var = .x, 
    design = vacsDesign,
    filter_expr = Age13to17 == 1 & CSEC == 1
...
createKable(CSECCount, "Counts of Sexually Active Respondents Ages 13-17 based on Type of Transactional Sex and whether or not Transactional Sex was with a Tourist")
```
**Table Key:**
...
CSECProp <- c("SexTransactType", "SexTransactTourist") %>% 
    map( ~ calculateProp(
    var = .x, 
    design = vacsDesign,
    filter_expr = Age13to17 == 1 & CSEC == 1
  ...
  list_rbind    
createKable(CSECProp, "Proportions of Sexually Active Respondents Ages 13-17 based on Type of Transactional Sex and whether or not Transactional Sex was with a Tourist")
...
## b) Subset of sexually active respondents ages 18 Plus who reported a personal history of CSEC & transactional sex variables

```{r PropCountCSECAges18Plus, asis = TRUE}
...
CSECProp <- c("CSEC") %>% 
  ...
  list_rbind    
createKable(CSECProp, "Proportions of CSEC Among Sexually Active Respondents Ages 18 Plus who Answered Question about Transactional Sex")
...
createKable(CSECCount, "Counts of CSEC Responses among Sexually Active Respondents Ages 18 Plus who Answered Question about Transactional Sex")
...
createKable(CSECAgeDescriptives, "Mean Ages of Victim and Perpetrator for Sexually Active Respondents Ages 18 Plus who Reported Transactional Sex (CSEC)")
```
...
createKable(CSECCount, "Counts of Sexually Active Respondents Ages 18 Plus based on Type of Transactional Sex and whether or not Transactional Sex was with a Tourist")
```
...
CSECProp <- c("SexTransactType", "SexTransactTourist") %>% 
  ...
  list_rbind    
createKable(CSECProp, "Proportions of Sexually Active Respondents Ages 18 Plus based on Type of Transactional Sex and whether or not Transactional Sex was with a Tourist")

```

# **4. Main Analysis**

Performs the main analyses on CSEC among respondents and associations with demographic, psycho-social, and environmental factors. Analysis is filtered by age.

## a) Descriptives, Proportions, and Counts

Proportions as well as their SE and CIs were all multiplied by 100 and represent the estimated percentages of the target population.

### i) Filtered: Sexually active respondents ages 13 to 17 who answered question about transactional sex question
...
createKable(ContDescriptives, "Means of Continuous Variables filtered by sexually active respondents Ages 13 to 17 who answered question about transactional sex")
...
createKable(CatDescriptives, "Proportions for Categorical and Binarized Variables filtered by sexually active respondents Ages 13 to 17 who answered question about transactional sex")
```
...
createKable(ContDescriptives, "Means of Continuous Variables filtered by sexually active respondents Ages 18Plus who reported CSEC")
...
```{r chiSquFunctions, results='asis'}
...
chisq_table2 <- function(var, group, design, filter_expr = NULL, digits = 4) {
    ...
    if (length(row_labels) == 2 && all(row_labels %in% c("0", "1"))){
        row_labels <- ifelse(row_labels == "1", "Yes", "No")
        rownames(obs) <- row_labels
        rownames(exp) <- row_labels
        rownames(res) <- row_labels
        rownames(std) <- row_labels
    }
    
    table_data <- data.frame(Variable = row_labels)
    names(table_data)[1] <- var_name     
...
|  |  |  |
|--------------|------------|----------------|
| F | F-statistic from Adjusted Wald test | Test of independence that measures whether estimate of association parameters are zero; Provides better adjustments for variable comparisons, especially if the data has a small number of PSUs compared to degrees of freedom; larger values suggest stronger association |
