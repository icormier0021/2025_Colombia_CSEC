...
The cleaned data-set ready for analysis is saved under '/data' as "Colombia_VACS_National_data_CLEANED.sav". To review the process used for reviewing and preparing the raw data, please review the RMarkdown report "/report/CSEC_Colombia_National_Data_Clean-Prep.Rmd". 

# **1. Load Data**

```{r loadData}
df <- read_sav(here::here("data", "Colombia_VACS_National_data_CLEANED.sav"))
```
...
**Mean & Totals Tables (Continuous Variables)**

| Column Label | Definition | Interpretation |
|--------------|------------|----------------|
| Variable | Variable name from dataset | Identifies the specific variable being analyzed |
| Variable Label | Descriptive label for the variable | Human-readable description of what the variable measures |
| Mean | Weighted population mean | Average value in the target population, accounting for survey weights |
| Mean(SE) | Standard error of the mean | Measure of precision; smaller values indicate more precise estimates |
| Mean(CV) | Coefficient of variation of the mean | Relative measure of precision (SE/Mean); values <0.30 generally considered reliable |
...
| Mean(DeEff) | Design effect for the mean | Ratio of actual variance to simple random sample variance; >1 indicates clustering/stratification increases variance |
...
| TotalNA | Unweighted count of missing values | Number of survey respondents who did not answer this question |
| Total | Weighted population total | Estimated total in the target population, accounting for survey weights |
| Total(SE) | Standard error of the total | Measure of precision for the population total estimate |
| Total(CV) | Coefficient of variation of the total | Relative measure of precision for the total (SE/Total) |
| Total(Low CI 95%) | Lower bound of 95% CI for total | We can be 95% confident the true population total is above this value |
| Total(Upp CI 95%) | Upper bound of 95% CI for total | We can be 95% confident the true population total is below this value |

**Count Tables (Categorical Variables)**
| Column Label | Definition | Interpretation |
|--------------|------------|----------------|
| Variable | Variable name from dataset | Identifies the specific variable being analyzed |
| Variable Label | Descriptive label for the variable | Human-readable description of what the variable measures |
| Answer | Response category | Shows "Yes"/"No" or other categorical response options |
| N | Weighted count estimate | Estimated number of people in this category in the target population |
| N(SE) | Standard error of the count | Measure of precision for the count estimate |
...
| N(CV) | Coefficient of variation of the count | Relative measure of precision (SE/N); values <0.30 generally considered reliable |

**Proportion Tables (Categorical Variables)**
...
| Variable Label | Descriptive label for the variable | Human-readable description of what the variable measures |
| Answer | Response category | Shows "Yes"/"No" or other categorical response options |
| Prop | Weighted proportion (as percentage) | Estimated percentage of the target population in this category |
