---
title: "CSEC in Colombia: Priority Stratum- Data Prep & Cleaning"
...
```{r loadData}
VACSData <- read_sav(here::here("rawdata", "Colombia_VACS_priority_data.sav"))
```
...
rawDF <- VACSData %>%
  select(
    DEPT, MUNICIPIO, PSU, HH, VACS_ID, Wght_Final, H6, H7H, H18, H19, H19A, H54, H56, H58, H19B,
...
Notes: 
- Q306 (BullyVic) seems missing across both Sexes
...
Notes: 
- Q306 (BullyVic) seems missing across all Ages
- Q58 (Partner) and Q7AA (FoodSecure) seems to have less missing values for younger individuals
...
df <- set_variable_labels(df,
                            ...
                            DEPT = "Department",
                            MUNICIPIO = "Municipality/Commune",
...
```{r skimDF, echo=FALSE}
SkimData <- df %>% 
        select(- c("VACS_ID","Wght_Final","PSU","DEPT","MUNICIPIO","HH"))
...
The cleaned and structured dataset ready for analysis is saved under the sub-directory 'data', and is named "Colombia_VACS_Priority_data_CLEANED". It is saved in '.rds'(R's native file format), '.sav' and '.csv' format.
```{r saveData}

write.csv(df, here::here("data", "Colombia_VACS_Priority_data_CLEANED.csv"), row.names = FALSE)
write_sav(df, here::here("data", "Colombia_VACS_Priority_data_CLEANED.sav"))
saveRDS(df, here::here("data","Colombia_VACS_Priority_data_CLEANED.rds"))
```
